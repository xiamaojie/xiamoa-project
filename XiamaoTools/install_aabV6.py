"""
安装 AAB 第 6 版本脚本，支持获取应用名称，版本号，自动保存上一次版本号，检查权限，检查手机时间和服务器等功能。
"""
import os
import re
import subprocess
...
def check_time_diff(max_diff_seconds=60, ntp_timeout=5):
    ...
    for host in NTP_SERVERS:
        ...
        try:
            response = client.request(host, version=3, timeout=ntp_timeout)
            ...
            print(f"NTP 服务器不可用: {host}，原因: {ntp_err}，耗时: {elapsed:.1f}s（超时 {ntp_timeout}s 后切换下一个）")
...
def start_app(package_name: str) -> None:
    """智能启动应用：Launcher → 扩展 Splash → Monkey"""
    print("🚀 尝试启动应用...")
...
def install_aab(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        except (FileNotFoundError, RuntimeError, ValueError) as exe:
