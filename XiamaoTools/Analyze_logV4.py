...
product_requirement_event_name = [
    ...
    {"Loading_Page_Show": ["Type"]},  # 注意：这里有两个相同的事件名，但根据要求按顺序排列
    ...
    for line in lines:
        for event_config in product_requirement_event_name:
            if isinstance(event_config, str):
                pattern = r'origin=app,name=' + event_config + r','
                if re.search(pattern, line) and event_config not in reported_events:
                ...
                pattern = r'origin=app,name=' + event_name + r','
                if re.search(pattern, line):
                    ...
                    if params_match:
                        ...
                        for attr in attributes:
                            attr_pattern = r'' + re.escape(attr) + r'=' + r'([^,\}]+)'
                            attr_match = re.search(attr_pattern, params_str)
            ...
            if attr_list and event_name in reported_set:
                ...
                for attr in attr_list:
                    ...
                    if unique_vals:
                        ...
                        unique_values[attr] = ["null"]  # 保留 "null" 如果没有有效值
                # 构建去重后的格式化值
